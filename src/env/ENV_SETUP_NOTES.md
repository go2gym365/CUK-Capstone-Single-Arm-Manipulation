...
- ROBOTIS OMX 관련 코드/에셋 중심을 `src/env/ROBOTIS_OMX_AI/env_ms/`로 정리
...
- `c96eb11` (`feat(env): consolidate cube-bottle scene and docs`)
  ...
  - 파일 경로 재정리(`src/env` -> `src/env/ROBOTIS_OMX_AI/env_ms`)
  - GUI 마커 기본 OFF, 문서(`ENV_SETUP_NOTES.md`) 추가

## 3. 씬 구조 변경
### 3.1 유지/추가/삭제
- 유지: `src/env/ROBOTIS_OMX_AI/env_ms/scene.xml` (기본 베이스 씬)
- 추가: `src/env/ROBOTIS_OMX_AI/env_ms/scene_cube_bottle.xml` (공통 작업 씬)
- 삭제: `src/env/ROBOTIS_OMX_AI/env_ms/scene_reach.xml`
...
## 5. 실행 스크립트/기본값 변경
### 5.1 GUI 실행 스크립트
파일: `src/env/ROBOTIS_OMX_AI/env_ms/runners/view_robotis.py`
...
### 5.2 환경 기본 XML
파일: `src/env/ROBOTIS_OMX_AI/env_ms/robotis_env.py`
- `xml_path` 미지정 시 기본값:
  - `src/env/ROBOTIS_OMX_AI/env_ms/scene_cube_bottle.xml`

### 5.3 설정 파일 경로
파일: `configs/env/reach.yaml`
- `xml_path: src/env/ROBOTIS_OMX_AI/env_ms/scene_cube_bottle.xml`
...
.venv/bin/python -m src.env.ROBOTIS_OMX_AI.env_ms.runners.view_robotis --scene cube_bottle
```

```bash
# 2) 마커 포함 GUI 실행 (site group 0 ON)
.venv/bin/python -m src.env.ROBOTIS_OMX_AI.env_ms.runners.view_robotis --scene cube_bottle --show-site-group0
```

```bash
# 3) 로드 확인만 수행 (GUI 미오픈)
.venv/bin/python -m src.env.ROBOTIS_OMX_AI.env_ms.runners.view_robotis --scene cube_bottle --dry-run
```

```bash
# 4) 환경 스모크 테스트
.venv/bin/python -m src.env.ROBOTIS_OMX_AI.env_ms.runners.smoke_test --steps 5
```

## 7. 현재 상태 체크리스트 (이 문서 기준 최신 기준 파일)
- [ ] `src/env/ROBOTIS_OMX_AI/env_ms/scene_cube_bottle.xml` 존재
- [ ] `src/env/ROBOTIS_OMX_AI/env_ms/scene.xml` 존재
- [ ] `src/env/ROBOTIS_OMX_AI/env_ms/scene_reach.xml` 제거됨
- [ ] `src/env/ROBOTIS_OMX_AI/env_ms/runners/view_robotis.py`에 `--scene cube_bottle` 기본값 적용
- [ ] `src/env/ROBOTIS_OMX_AI/env_ms/runners/view_robotis.py`에 site group 0 기본 OFF 적용
- [ ] `src/env/ROBOTIS_OMX_AI/env_ms/robotis_env.py` 기본 XML이 `scene_cube_bottle.xml`
